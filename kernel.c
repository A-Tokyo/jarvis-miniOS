...
//  buffer2[3]='o';
//
// for(i=4; i<13312; i++) buffer2[i]=0x0;
// makeInterrupt21();
...
for (;secCounter < secNum && (generalpointer< rowPointer + 32) ; secCounter++ , generalpointer++) {
      ...
      for (; mapPointer < 512   ; mapPointer++ ) {
         if(map[mapPointer] == '\0'){
          ...
          for (; partOfBufferPointer < 512; bufferPointer++ , partOfBufferPointer++) {
...
void ListFile(){
	...
	for(i=0;i<16;i++){
		...
		if (dirBuff[32*i] != 0x0){
			...
			for (j=0; j < 6; j++){
        if( dirBuff[32*i] == 0x00) break;
				displayBuff[index] = dirBuff[j+32*i];
				index++;
			}
      ...
      for (j=6; j < 32; j++){
        if(dirBuff[j+32*i] == 0x00) break;
      }
      // add no of sec
 displayBuff[index] = 0x20;
index++;

      if((j-6) == 0)
       displayBuff[index] = 0x31;
        else if ((j-6) == 1)  displayBuff[index] = 0x31;
        else if ((j-6) == 2)  displayBuff[index] = 0x32;
        else if ((j-6) == 3)  displayBuff[index] = 0x33;
        else if ((j-6) == 4)  displayBuff[index] = 0x34;
        else if ((j-6) == 5)  displayBuff[index] = 0x35;
        else if ((j-6) == 6)  displayBuff[index] = 0x36;
        else if ((j-6) == 7)  displayBuff[index] = 0x37;
        else if ((j-6) == 8)  displayBuff[index] = 0x38;
        else if ((j-6) == 9)  displayBuff[index] = 0x39;
        else if ((j-6) == 10){displayBuff[index] = 0x31; index++; displayBuff[index] = 0x30;}
        else if ((j-6) == 11){displayBuff[index] = 0x31; index++; displayBuff[index] = 0x31;}
        else if ((j-6) == 12){displayBuff[index] = 0x31; index++; displayBuff[index] = 0x32;}
        else if ((j-6) == 13){displayBuff[index] = 0x31; index++; displayBuff[index] = 0x33;}
        else if ((j-6) == 14){displayBuff[index] = 0x31; index++; displayBuff[index] = 0x34;}
        else if ((j-6) == 15){displayBuff[index] = 0x31; index++; displayBuff[index] = 0x35;}
        else if ((j-6) == 16){displayBuff[index] = 0x31; index++; displayBuff[index] = 0x36;}
        else if ((j-6) == 17){displayBuff[index] = 0x31; index++; displayBuff[index] = 0x37;}
        else if ((j-6) == 18){displayBuff[index] = 0x31; index++; displayBuff[index] = 0x38;}
        else if ((j-6) == 19){displayBuff[index] = 0x31; index++; displayBuff[index] = 0x39;}
        else if ((j-6) == 20){displayBuff[index] = 0x32; index++; displayBuff[index] = 0x30;}
        else if ((j-6) == 21){displayBuff[index] = 0x32; index++; displayBuff[index] = 0x31;}
        else if ((j-6) == 22){displayBuff[index] = 0x32; index++; displayBuff[index] = 0x32;}
        else if ((j-6) == 23){displayBuff[index] = 0x32; index++; displayBuff[index] = 0x33;}
        else if ((j-6) == 24){displayBuff[index] = 0x32; index++; displayBuff[index] = 0x34;}
        else if ((j-6) == 25){displayBuff[index] = 0x32; index++; displayBuff[index] = 0x35;}
        else if ((j-6) == 26){displayBuff[index] = 0x32; index++; displayBuff[index] = 0x36;}

index++;
displayBuff[index] = 0x20;
index++;

displayBuff[index] = 'S';index++;
displayBuff[index] = 'e';index++;
displayBuff[index] = 'c';index++;
displayBuff[index] = 't';index++;
displayBuff[index] = 'o';index++;
displayBuff[index] = 'r';index++;
displayBuff[index] = 's';index++;

      // end of adding no of sec
			/* Add some formatting */
			displayBuff[index] = '\r';
			index++;
			displayBuff[index] = '\n';
